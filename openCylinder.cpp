...
namespace static_meshes_3D {
	...
	OpenCylinder::OpenCylinder(float radius, int numSlices, float height, bool withPositions, bool withNormals, bool withTextureCoordinates)
		: StaticMesh3D(withPositions, withNormals, withTextureCoordinates)
		, _radius(radius)
	...
	float OpenCylinder::getRadius() const
	{
		return _radius;
	}

	int OpenCylinder::getSlices() const
	{
		return _numSlices;
	}

	float OpenCylinder::getHeight() const
	{
		return _height;
	}

	void OpenCylinder::initializeData()
	{
		if (_isInitialized) {
			return;
		}
		...
		if (hasPositions())
		{
			...
			// Add OpenCylinder side vertices
			for (auto i = 0; i <= _numSlices; i++)
			{
		...
		if (hasTextureCoordinates())
		{
			...
			// I have decided to map the texture twice around OpenCylinder, looks fine
			...
			//// Generate circle texture coordinates for OpenCylinder top cover
			...
			// Generate circle texture coordinates for OpenCylinder bottom cover
			//glm::vec2 topBottomCenterTexCoord(0.5f, 0.5f);
			_vbo.addData(topBottomCenterTexCoord);
			for (auto i = 0; i <= _numSlices; i++) {
				_vbo.addData(glm::vec2(topBottomCenterTexCoord.x + sines[i] * 0.5f, topBottomCenterTexCoord.y - cosines[i] * 0.5f));
		...
		if (hasNormals())
		{
			...
			// Add normal for every vertex of OpenCylinder top cover
			_vbo.addData(glm::vec3(0.0f, 1.0f, 0.0f), _numVerticesTopBottom);

			// Add normal for every vertex of OpenCylinder bottom cover
			_vbo.addData(glm::vec3(0.0f, -1.0f, 0.0f), _numVerticesTopBottom);
		}
	...
	void OpenCylinder::render() const
	{
		if (!_isInitialized) {
			return;
		}
		...
		// Render OpenCylinder side first
	...
	void OpenCylinder::renderPoints() const
	{
		if (!_isInitialized) {
			return;
		}
